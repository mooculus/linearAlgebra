...
\begin{proof}
The consistency theorem for matrix equations tells us that the matrix equation $A*{\bf x} = {\bf b}$ if and only if $\bf b$ can be written as a linear combination of the columns of $A$. But this is equivalent to saying that $\bf b$ is in the span of the columns of $A$; that is, ${\bf b}\in C(A)$.
\end{proof}
...
-1 & 3 & 7 & 1 & 4\\
2 & 6 & 9 & 8 & -3
\end{bmatrix}$, compute a minimal spanning set for $Range(A)\subseteq \mathbb R^4$. Determine if $Range(A) = \mathbb R^4$, or if it is a proper subset.
\end{exercise}
