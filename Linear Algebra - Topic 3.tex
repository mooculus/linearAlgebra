...
\begin{lemma}\label{lemma:indep} A collection of vectors $\{{\bf v}_1,\dots,{\bf v}_n\}$ is linearly independent iff it is vectorwise independent.
\end{lemma}
...
Most linear algebra texts do not discuss the notions of linear combinations and linear dependence when the number of vectors involved is infinite. However, there are a number of benefits of being able to extending to this case. Thus, suppose $\{{\bf v}_\alpha\}$ is an arbitrary (possibly infinite) collection of vectors.

\begin{definition} A {\it linear combination} of a collection of vectors $S = \{{\bf v}_\alpha\}\subset V$ is a sum which can be written as a linear combination of some {\it finite} subset of $S$ (alternatively, a sum of the form $\sum \lambda_\alpha {\bf v}_{\alpha}$ where $\lambda_\alpha = 0$ except for {\it finitely} many $\alpha$).
\end{definition}

Similarly,

\begin{definition} A collection of vectors $S = \{{\bf v}_\alpha\}\subset V$ is {\it linearly independent} resp.\ {\it vectorwise independent} iff every finite subset of vectors in $S$ is linearly independent resp.\ vectorwise independent (as defined above).
\end{definition}

\begin{exercise} State and prove the generalization of Lemma \ref{lemma:indep} for arbitrary collections of vectors.
\end{exercise}
...
In other words, for any subset $S\subset V$, $Span(S) =$ the set of all linear combinations of elements of $S$.
...
\subsubsection{Row space, column space, and codomain} Let $A\in \mathbb R^{m\times n}$. One may construct from $A$ various different subspaces, each of which is important in its own right.
\vskip.2in

{\bf\underbar{The row space of $A$}} Each row of $A$ is a $1\times n$ vector in $\mathbb R^{1\times n}$, and the row space is the span of this collection of vectors:
\[
Row(A) := Span\{A(i,:)\}_{1\le i\le m}\subseteq \mathbb R^{1\times n}
\]

Similarly one has
\vskip.2in

{\bf\underbar{The column space of $A$}} Each column of $A$ is an $m\times 1$ vector in $\mathbb R^{m\times 1} = \mathbb R^m$, and the column space is the span of this collection of vectors:
\[
Col(A) := Span\{A(:,j)\}_{1\le i\le n}\subseteq \mathbb R^{m}
\]

Finally, the 
\vskip.2in

{\bf\underbar{The codomain, or range of $A$}} (viewed as a linear transformation, defined below) is
\[
...
\begin{lemma} For any real $m\times n$ matrix $A$, $Row(A)$ is a subspace of $\mathbb R^{1\times n}$, while $Col(A) = Range(A)$ is a subspace of $\mathbb R^m$.
\end{lemma}

\begin{proof} The span of any collection of vectors is a subspace of the space in which they lie. Now we have seen that for any ${\bf x} = [x_1\ x_2\dots x_n]^T\in \mathbb R^n$,
\[
A*{\bf x} = \sum_{j=1}^n x_j A(:,j)\in\mathbb R^m
\]
This equality identifies the range and column space of $A$.
\end{proof}
\vskip.3in
...
\subsection{Bases and dimension} A set of vectors $S = \{{\bf v}_\alpha\}\subset V$ is a {\it basis} for $V$ if
\begin{itemize}
\item it spans $V$ and
\item it is linearly independent.
\end{itemize}

The concept of a basis makes perfect sense regardless of the number of vectors in $S$
\vskip.2in

{\bf\underbar{Fundamental Properties}}

\begin{itemize}
\item[[B1]] $S = \{{\bf v}_\alpha\}\subset V$ is a basis for $V$ iff it is a minimal spanning set for $V$.
\item[[B2]] Every non-zero vector space $V$ admits a basis.
\item[[B3]] If $S = \{{\bf v}_\alpha\}$ and $T = \{{\bf w}_\beta\}$ are two bases for $V$, then $|S|=|T|$.
\end{itemize}
\vskip.2in
