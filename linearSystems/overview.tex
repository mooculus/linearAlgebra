...
\begin{definition}
  ...
  \end{alignat*}
  and a \dfn{solution} to that system is an assignment of values to the variables which make \dfn{each equation} hold true. The \dfn{solution set} of a system of equations is the collection, or set of all the solutions of that system (which will be empty if the system has no solutions). Given $m$ equations in the same set of $n$ unknowns, the ordered pair of integers $(m,n)$ are referred to as the \dfn{dimensions} of the system, and the system would be referred to as an \dfn{$m\times n$ system of equations} (read: \dfn{m-by-n system of equations}), or an \dfn{$m\times n$ system} for short.
\end{definition}
...
An important concept involved in answering these questions is that of equivalence. We will say that two systems in the same set of unknowns are \dfn{equivalent} if they have the same set of solutions.
